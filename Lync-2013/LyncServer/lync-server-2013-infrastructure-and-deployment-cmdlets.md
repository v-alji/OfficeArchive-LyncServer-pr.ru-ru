...
Администраторам часто приходится напрямую вызывать многие из инфраструктуры и развертывания. Это связано с тем, что эти командлеты автоматически вызываются при запуске программы установки или построителе топологии. (Одно из серьезных исключений может быть командлетом **Export-CsConfiguration** , который позволяет создавать резервные копии топологии сервера Lync, политик и параметров конфигурации.) Тем не менее, и при необходимости командлеты инфраструктуры и развертывания также можно запускать из командной консоли Lync Server Management Shell или из сценария. Использование сценария позволяет автоматизировать определенные задачи. Ниже приведен список командлетов, которые непосредственно связаны с инфраструктурой и развертыванием.
