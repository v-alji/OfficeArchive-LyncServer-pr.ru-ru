...
> [!NOTE]  
> Вы можете использовать командную консоль Lync Server или средство настройки группы ответа для создания и изменения рабочих процессов группы слежения. Вы можете получить доступ к средству настройки группы ответа из панели управления Lync Server или открыть ее непосредственно из веб-браузера, введя следующий URL-адрес: <STRONG>https://</STRONG> &lt; webPoolFqdn &gt; <STRONG>/RgsConfig</STRONG>.
...
5.  В поле поиска **Выбор службы** полностью или частично введите имя службы **ApplicationServer**, обеспечивающей размещение создаваемого или изменяемого рабочего процесса. В сформированном списке служб щелкните требуемую службу, затем нажмите кнопку **ОК**.
    ...
    > [!NOTE]  
    > Откроется средство настройки группы ответа. Вы также можете открыть средство настройки группы ответа прямо из веб-браузера, введя следующий URL-адрес: <STRONG>https://</STRONG> &lt; webPoolFqdn &gt; <STRONG>/RgsConfig</STRONG>.
...
19. Выберите тип рабочего графика, который следует использовать. Для этого выполните одно из следующих действий:
      ...
      - Для применения предварительно заданного графика рабочего времени щелкните **Использовать готовое расписание**, затем выберите требуемое расписание в раскрывающемся списке.
        ...
        > [!NOTE]  
        > Чтобы можно было выбрать этот параметр, необходимо предварительно определить хотя бы одно предварительно заданное расписание. Готовые расписания определяются с помощью командлета <STRONG>New-CSRgsHoursOfBusiness</STRONG> . Дополнительные сведения можно найти <A href="lync-server-2013-optional-define-response-group-business-hours.md">в разделе (необязательно) определение группы ответа в часах Lync Server 2013</A>.
...
24. В разделе **Шаг 5. Указание выходных дней** установите флажки для одной или нескольких групп выходных дней, когда группа ответа не будет работать.
    ...
    > [!NOTE]  
    > Перед настройкой рабочего процесса вам нужно определить праздники и наборы праздников. Используйте командлеты <STRONG>New-CsRgsHoliday</STRONG> и <STRONG>New-CsRgsHolidaySet</STRONG> для определения праздников и наборов праздников. Дополнительные сведения можно найти <A href="lync-server-2013-optional-define-response-group-holiday-sets.md">в разделе (необязательно) Определение наборов праздников группы ответа в Lync Server 2013</A>.
...
4.  Получите идентификатор очереди или вопроса при перенаправлении вызовов. В командной строке выполните команду:
    ...
    Подробнее о создании очереди можно узнать в статьях [New-CsRgsQueue](https://docs.microsoft.com/powershell/module/skype/New-CsRgsQueue).
...
9.  Создайте или измените рабочий процесс. Для создания рабочего процесса выполните командлет **New-CsRgsWorkflow**. Для изменения рабочего процесса выполните командлет **Set-CsRgsWorkflow**. В командной строке введите следующую команду:
    ...
    > [!NOTE]  
    > Дополнительные сведения о дополнительных параметрах можно найти в статьях <A href="https://docs.microsoft.com/powershell/module/skype/New-CsRgsWorkflow">New-CsRgsWorkflow</A> или <A href="https://docs.microsoft.com/powershell/module/skype/Set-CsRgsWorkflow">Set-CsRgsWorkflow</A>
