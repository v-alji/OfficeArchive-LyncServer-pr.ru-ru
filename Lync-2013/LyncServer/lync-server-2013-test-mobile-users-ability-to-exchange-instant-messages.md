...
Если два тестовых пользователя могут обмениваться мгновенными сообщениями с помощью службы Mobility Service, Test-CsMcxP2PIM будут возвращать результаты проверки.
...
Внутреннее исключение: запрос HHTP не разрешен
...
Внутренняя диагностика: X-MS-Server-Fqdb: ATL-CS-
...
Если Test-CsMcxP2PIM не удается выполнить первый шаг, необходимо проверить, работает ли служба Mobility Service. Это можно сделать с помощью веб-браузера, чтобы убедиться, что вы можете получить доступ к URL-адресу службы Mobility Service для пула Lync Server. Например, эта команда проверяет URL-адрес для пула atl-cs-001.litwareinc.com:
