...
Если Test-CsIM не удается, возможно, потребуется повторно выполнить тест, на этот раз включая параметр подробно:

    Test-CsIM -TargetFqdn "atl-cs-001.litwareinc.com" -Verbose

После включения подробной информации Test-CsIM будет возвращать пошаговые учетные записи каждого действия, которое оно предпринимало, когда вы проверили возможности двух тестовых пользователей стать частью сеанса обмена мгновенными сообщениями. Например, в приведенном ниже примере показано, как выводится сообщение о неправильном наборе учетных данных пользователя (в данном случае — неверный пароль) для проверки-CsIM:
...
Ниже приведены некоторые распространенные причины, по которым может произойти сбой Test-CsIM.
  ...
  - Возможно, служба мгновенных сообщений недоступна. С помощью Lync Server вы можете настроить систему таким образом, чтобы мгновенное сообщение не помогло быть доступно, если база данных архивации недоступна. Это можно проверить, выполнив команду, подобную следующей:
    ...
    Если для BlockOnArchiveFailure задано значение "true", необходимо определить, доступна ли база данных архивации. Вы можете вернуть расположение баз данных архивации с помощью следующей команды:
