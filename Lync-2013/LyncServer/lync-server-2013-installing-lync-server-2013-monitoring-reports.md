...
4.  На странице **Определение учетных данных** в поле **имя пользователя** введите доменное имя и имя пользователя учетной записи, которая будет использоваться для доступа к отчетам мониторинга (например, **плана litwareinc \\ kenmyer**). Если вы не используете этот формат (доменное \\ имя пользователя), произойдет ошибка.
...
Отчеты мониторинга также можно установить из командной консоли Lync Server, запустив сценарий DeployReports.ps1; Этот сценарий Windows PowerShell можно найти на установочном носителе Lync Server в \\ \\ папке Setup ReportingSetup. To install Monitoring Reports using DeployReports.ps1, type a command similar to the following at the Management Shell prompt:
